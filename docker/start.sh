...
if [ ! -f /usr/share/nginx/www/wp-config.php ]; then
  ...
  sed -i "/DB_HOST/s/'[^']*'/'mysql-connect'/2" /usr/share/nginx/www/wp-config.php
...
ENDL
  ...
  #  wp --allow-root config create --dbname=new --dbuser=krishna --dbpass=krishna --dbhost=localhost 
    wp --allow-root core install --url=$TEST  --title=$DEVOPS --admin_user=$USER --admin_email=$EMAIL --admin_password=$PASSWORD  
